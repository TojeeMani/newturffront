...
const EnhancedChatWidget = () => {
  ...
  const [messages, setMessages] = useState(() => {
    ...
    return [{ 
      id: 'welcome', 
      role: 'assistant', 
      content: "Hi! I'm your advanced TurfEase assistant. I can help with bookings, prices, availability, and much more. What would you like to know?", 
      ts: Date.now(),
      intent: 'greeting',
      suggestions: ['Browse turfs', 'Check prices', 'Find nearby', 'My bookings']
    }];
  });
  ...
  useEffect(() => {
    if (user?.id) {
      const context = chatService.loadConversationContext(user.id);
      if (context) {
        setConversationContext(context);
        setSmartSuggestions(chatService.generateSmartSuggestions({
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      if (responseData.intent) {
        const newSmartSuggestions = chatService.generateSmartSuggestions({
          userType: user?.userType,
          lastIntent: responseData.intent
        });
      ...
      <div key={message.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'} animate-in fade-in duration-300`}>
        <div className={`max-w-[85%] ${isUser ? 'order-2' : 'order-1'}`}>
          <div className={`text-[10px] mb-1 ${isUser ? 'text-right text-gray-500' : 'text-gray-500'}`}>
            {isUser ? (user?.firstName || 'You') : 'Assistant'} · {new Date(message.ts || Date.now()).toLocaleTimeString()}
    ...
    <div className="fixed bottom-6 right-6 z-50">
      {isOpen && (
        <div className="w-80 md:w-96 h-[32rem] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden mb-3 animate-in slide-in-from-bottom-2 duration-300">
          ...
          {/* Smart Suggestions */}
          {!loading && (suggestions.length > 0 || smartSuggestions.length > 0) && (
            <div className="px-4 pb-2">
              ...
              <div className="flex flex-wrap gap-2">
                {(suggestions.length > 0 ? suggestions : smartSuggestions).slice(0, 4).map(s => (
                  <button 
                    key={s} 
